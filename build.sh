...
DEVICE="X01BD"
KERNEL="Chronox"
CAFTAG="LA.UM.10.2.1.r1-04300-sdm660.0"
KERNELTYPE="v4.19"
...
clone() {
	...
	if [[ $COMPILER == "clang" ]]; then
		# Clone clang
		git clone https://gitlab.com/RyuujiX/neutron-clang -b Neutron-16 clang
...
build_kernel() {
	...
	if [[ $COMPILER == "clang" ]]; then
		make -j"$PROCS" O=out \
				...
				HOSTCC=clang \
				HOSTCXX=clang++ AR=llvm-ar NM=llvm-nm AS=llvm-as STRIP=llvm-strip OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump READELF=llvm-readelf HOSTAR=llvm-ar HOSTAS=llvm-as LD_LIBRARY_PATH=$clangDir/lib LD=ld.lld HOSTLD=ld.lld
	fi
